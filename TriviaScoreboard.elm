...
getNth n entry =
  let
    value = List.drop (n - 1) entry |> List.head
  in
    case value of
      Just value -> value
      Nothing -> ""
...
updateScore appendOrOverwrite scoreString teamToEdit teamsAndScores =
  let
    remainingTeams = teamsAndScores
                   |> List.filter (\e -> (getNth 1 e) /= teamToEdit)
    teamToUpdate = teamsAndScores
                 |> List.filter (\e -> (getNth 1 e) == teamToEdit)
                 |> getNucleus
    spaceOrNot = if List.length teamToUpdate == 1 then "" else " "
    replacedEntry = [[ teamToEdit, scoreString ]]
    appendedEntry = [[ teamToEdit, (getNth 2 teamToUpdate)
                                   ++ spaceOrNot
                                   ++ scoreString ]]
  in
    case appendOrOverwrite of
      "overwritescores" -> List.append remainingTeams replacedEntry
      "deleteteam" -> remainingTeams
      _ -> List.append remainingTeams appendedEntry


blockDuplicateTeam : String -> List ( List String ) -> List ( List String )
blockDuplicateTeam teamName teamsAndScores =
  let
    listOfTeams = List.map (\e -> getNth 1 e) teamsAndScores
  in
...
update msg model =
  case msg of
    EnterTeamName s ->
      { model | teamName = s }
    AddTeam ->
      { model | teamsAndScores = blockDuplicateTeam model.teamName
    ...
    UpdateScoreString s ->
      { model | score = s }
    EnterEditScore ->
      { model | teamsAndScores = updateScore "overwritescores"
                                             model.score
                                             model.teamToEdit
                                             model.teamsAndScores }
    EnterAddScore ->
      { model | teamsAndScores = updateScore "addscore"
                                             model.score
                                             model.teamToEdit
                                             model.teamsAndScores }
    SortByTeamName ->
    ...
    SortByTotalScore ->
      { model | teamsAndScores = List.sortBy
                                 (\e -> getNth 2 e |> calculateTotal)
                                 model.teamsAndScores
                                 |> List.reverse }
    DeleteTeam ->
      { model | teamsAndScores = updateScore "deleteteam"
                                             ""
                                             model.teamToEdit
                                             model.teamsAndScores }
    ToggleInstructions ->
...
displayRow ranks indexedEntry =
  let
    ...
    resultTdStyle = if (rem idx 2) == 0 then
                      ...
                      resultsTdStyleOne
    teamName = getNth 1 entry
    score = getNth 2 entry
    total = score |> calculateTotal
    rankToUse = getRank total ranks
    resultTotalTdStyle = if (rem idx 2) == 0 then
                           resultsTdStyleZeroBold
                         else
                           resultsTdStyleOneBold
...
showTable teamsAndScores =
  let
    ranks = makeRankDict teamsAndScores
  in
